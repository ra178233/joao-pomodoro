...
<div>
  ...
  <div class="container">
    <div class="text-center">{{ minutos }}:{{ segundos }}</div>
  </div>
  <div class="container">
  ...
  <div class="container">
    <button v-if="!working" v-on:click="diminuirTempo">
      <i class="fas fa-minus"></i>
    </button>
    <button v-if="!working" v-on:click="alterarMode">mode</button>
    <button v-if="!working" v-on:click="aumentarTempo">
...
export default {
  ...
  methods: {
    ...
    iniciado() {
      return (
        this.segundos != "00" &&
        ((this.pomodoroMode && this.minutos != 25) ||
          (!this.pomodoroMode && this.minutos != 5))
      );
    },
    finalizado() {
